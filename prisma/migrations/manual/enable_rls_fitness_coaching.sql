...
CREATE POLICY "Coaches can create their own templates"
  ...
  WITH CHECK ((select auth.uid()) IN (SELECT user_id FROM public.coach_profiles WHERE id = coach_id));

CREATE POLICY "Coaches can update their own templates"
  ON public.coaching_workout_templates FOR UPDATE
  USING ((select auth.uid()) IN (SELECT user_id FROM public.coach_profiles WHERE id = coach_id));
...
CREATE POLICY "Users can create notifications for others"
  ...
  WITH CHECK ((select auth.uid()) = sender_id OR sender_id IS NULL);
...
CREATE POLICY "Athletes can update their pending form checks"
  ...
  USING ((select auth.uid()) = athlete_id AND status = 'PENDING');
