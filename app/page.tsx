    ...
    <>
      <style jsx global>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .nav-icon { width: 28px; height: 28px; opacity: 0.7; transition: all 0.2s ease; cursor: pointer; }
      ...
      <div className="crt-wrapper">
        ...
        <div className="page-content">
          <nav className="retro-nav"><div className="nav-bar"><span className="nav-logo">GAMIFY.IT.COM</span><div className="nav-icons"><a href="https://fitness.gamify.it.com" target="_blank" rel="noopener noreferrer" title="Iron Quest"><DumbbellIcon className="nav-icon" /></a><a href="https://today.gamify.it.com" target="_blank" rel="noopener noreferrer" title="Day Quest"><ChecklistIcon className="nav-icon" /></a><a href="/travel" title="Explorer"><PlaneIcon className="nav-icon" /></a><LifeIcon className="nav-icon" style={{ opacity: 0.4, cursor: 'default' }} title="Life Tracker (Coming Soon)" /></div>{user && introComplete && (<div className="nav-center"><span>TOTAL XP:</span><span className="nav-xp-value">{totalXP.toLocaleString()}</span></div>)}{user ? (<div style={{ position: 'relative' }}><button onClick={() => setShowUserMenu(!showUserMenu)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>{user.user_metadata?.avatar_url ? (<img src={user.user_metadata.avatar_url} alt="Profile" className="profile-img" />) : (<div className="profile-img" style={{ background: 'linear-gradient(180deg, #FFD700 0%, #FFA500 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#1a1a1a', fontFamily: "'Press Start 2P', monospace", fontSize: '0.6rem' }}>{user.email?.charAt(0).toUpperCase()}</div>)}</button>{showUserMenu && (<div className="dropdown-menu"><div className="dropdown-email">{user.email}</div><button onClick={handleSignOut} className="dropdown-btn">SIGN OUT</button></div>)}</div>) : (<div style={{ position: 'relative' }}><button onClick={() => setShowLoginDropdown(!showLoginDropdown)} className="nav-btn">START PLAYING</button>{showLoginDropdown && (<div className="login-dropdown"><p className="login-label">Continue with</p><button onClick={handleGoogleSignIn} className="google-btn"><svg width="18" height="18" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>Google</button><div style={{margin:'1rem 0',borderTop:'1px solid #3a3a3a',position:'relative'}}><span style={{position:'absolute',top:'-0.5rem',left:'50%',transform:'translateX(-50%)',background:'#2d2d2d',padding:'0 0.5rem',fontSize:'0.3rem',color:'#666',fontFamily:"'Press Start 2P', monospace"}}>OR</span></div><form onSubmit={handleEmailLogin}><input type="email" placeholder="Email" value={email} onChange={(e)=>setEmail(e.target.value)} style={{width:'100%',padding:'0.5rem',marginBottom:'0.5rem',background:'#1a1a1a',border:'2px solid #3a3a3a',borderRadius:'4px',color:'#fff',fontFamily:"'Press Start 2P', monospace",fontSize:'0.32rem',boxSizing:'border-box'}}/><input type="password" placeholder="Password" value={password} onChange={(e)=>setPassword(e.target.value)} style={{width:'100%',padding:'0.5rem',marginBottom:'0.75rem',background:'#1a1a1a',border:'2px solid #3a3a3a',borderRadius:'4px',color:'#fff',fontFamily:"'Press Start 2P', monospace",fontSize:'0.32rem',boxSizing:'border-box'}}/><button type="submit" disabled={loginLoading} style={{width:'100%',padding:'0.6rem',background:loginLoading?'#666':'linear-gradient(180deg, #FFD700 0%, #FFA500 100%)',border:'2px solid #CC8800',borderRadius:'4px',color:'#1a1a1a',fontFamily:"'Press Start 2P', monospace",fontSize:'0.35rem',cursor:loginLoading?'wait':'pointer',boxShadow:'0 3px 0 #996600'}}>{loginLoading?'LOADING...':'LOGIN'}</button></form></div>)}</div>)}</div></nav>
          ...
          <section className="retro-section section-visible"><div className="section-header"><p className="section-label">// SELECT YOUR QUEST</p><h2 className="section-title shimmer-text">Choose Your Adventure</h2><p className="section-subtitle">Each app gamifies a different aspect of your life. Complete quests, earn XP, and level up across all domains.</p></div><div className="games-grid"><a href="https://fitness.gamify.it.com" target="_blank" rel="noopener noreferrer" className="game-card fitness"><span className="game-card-badge test">TEST</span><DumbbellIcon className="game-icon" /><h3 className="game-name">IRON QUEST</h3><p className="game-tagline">Turn every rep into XP</p><XPBar current={4250} max={5000} color="#FF6B6B" /><p className="game-domain">fitness.gamify.it.com</p></a><a href="https://today.gamify.it.com" target="_blank" rel="noopener noreferrer" className="game-card tasks"><span className="game-card-badge test">TEST</span><ChecklistIcon className="game-icon" /><h3 className="game-name">DAY QUEST</h3><p className="game-tagline">Conquer your daily missions</p><XPBar current={6800} max={10000} color="#5CC9F5" /><p className="game-domain">today.gamify.it.com</p></a><a href="/travel" className="game-card travel"><span className="game-card-badge test">TEST</span><PlaneIcon className="game-icon" /><h3 className="game-name">EXPLORER</h3><p className="game-tagline">Map your adventures</p><XPBar current={0} max={3000} color="#5fbf8a" /><p className="game-domain">gamify.it.com/travel</p></a><div className="game-card life" style={{ cursor: 'default', opacity: 0.7 }}><span className="game-card-badge coming-soon">SOON</span><LifeIcon className="game-icon" /><h3 className="game-name">LIFE TRACKER</h3><p className="game-tagline">Gamify everything else</p><XPBar current={0} max={1000} color="#a855f7" /><p className="game-domain">gamify.life</p></div></div></section>
