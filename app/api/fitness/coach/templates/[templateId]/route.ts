...
export const GET = withCoachAuthParams<{ templateId: string }>(
  async (_request, user, { templateId }) => {
...
export const PUT = withCoachAuthParams<{ templateId: string }>(
  async (request, user, { templateId }) => {
    const coach = await prisma.coach_profiles.findUnique({
      where: { user_id: user.id },
    });
    ...
    if (!existing) {
      return Errors.notFound("Template not found or not owned by you");
    }
...
export const DELETE = withCoachAuthParams<{ templateId: string }>(
  async (_request, user, { templateId }) => {
    const coach = await prisma.coach_profiles.findUnique({
      where: { user_id: user.id },
    });
