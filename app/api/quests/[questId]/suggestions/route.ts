...
export const GET = withAuthParams<{ questId: string }>(
  async (request, user, { questId }) => {
    ...
    const questNeighborhoodIds = quest.neighborhoods.map(
      (n) => n.neighborhood_id
    );
    ...
    if (scope === "neighborhood" && questNeighborhoodIds.length > 0) {
      ...
      scope === "city" ||
      (scope === "neighborhood" && questNeighborhoodIds.length === 0)
    ) {
    ...
    if (scope === "hotlist") {
      if (questCityIds.length === 0 && questNeighborhoodIds.length === 0) {
        return NextResponse.json({
          ...
          scope,
          questCities: quest.cities.map((c) => ({
            id: c.city.id,
            name: c.city.name,
            country: c.city.country,
          })),
          questNeighborhoods: quest.neighborhoods.map((n) => ({
    ...
    // Build hotlist map
    const hotlistByLocation = new Map<
      string,
      Array<{
        userId: string;
        username: string;
